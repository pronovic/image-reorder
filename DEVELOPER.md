...
**Import**) the `image-reorder` directory in PyCharm, then follow the remaining
...
Go to the PyCharm settings and find the `image-reorder` project.  Under **Project
Structure**, mark both `src` and `tests` as source folders.  In the **Exclude
Files** box, enter the following:

```
LICENSE;NOTICE;PyPI.md;.coverage;.coveragerc;.github;.gitignore;.gitattributes;.htmlcov;.idea;.isort.cfg;.mypy.ini;.mypy_cache;.pre-commit-config.yaml;.pylintrc;.pytest_cache;.pytest.ini;.tabignore;build;dist;out;poetry.lock;poetry.toml;run;.run;.venv;.runtime
```
...
Code is released to [PyPI](https://pypi.org/project/image-reorder/).  There is a
